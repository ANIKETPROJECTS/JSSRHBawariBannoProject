...
function BusinessTripsPage() {
  ...
  return <div><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.2em] text-gold">Travel register</p><h2 className="mt-1 font-display text-3xl text-primary">Business trips</h2><p className="mt-2 text-sm text-muted-foreground">Group travel, stay, food, and sourcing costs by trip.</p></div><button type="button" onClick={() => setEditing({ ...emptyBusinessTrip, startDate: new Date().toISOString().slice(0, 10) })} className="inline-flex items-center gap-2 bg-primary px-4 py-3 text-xs uppercase tracking-[0.14em] text-white"><Plus className="size-4" /> New business trip</button></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><MetricCard label="Trips recorded" value={trips.length} icon={MapPin} /><MetricCard label="Linked expenses" value={trips.reduce((sum, trip) => sum + Number(trip.expenseCount ?? 0), 0)} icon={CreditCard} /><MetricCard label="Total Expense (auto)" value={`₹${total.toLocaleString("en-IN")}`} icon={BarChart3} /></div><div className="mt-6 overflow-x-auto border border-[#ded5c9] bg-white"><table className="w-full min-w-[980px] text-left text-sm"><thead className="border-b border-border bg-[#fbf9f6] text-[10px] uppercase tracking-[0.14em] text-muted-foreground"><tr><th className="px-4 py-4">Trip ID</th><th className="px-4 py-4">Trip</th><th className="px-4 py-4">Dates</th><th className="px-4 py-4">Location</th><th className="px-4 py-4">Total Expense</th><th className="px-4 py-4">Purchase invoices</th><th className="px-4 py-4 text-right">Actions</th></tr></thead><tbody>{loading ? <tr><td colSpan={7} className="px-4 py-10 text-center text-muted-foreground">Loading business trips…</td></tr> : trips.length === 0 ? <tr><td colSpan={7} className="px-4 py-10 text-center text-muted-foreground">No business trips recorded yet.</td></tr> : trips.map((trip) => <tr key={trip._id} className="border-b border-border last:border-0 hover:bg-[#fbf9f6]"><td className="px-4 py-4 text-xs text-gold">{trip.tripId}</td><td className="px-4 py-4"><button type="button" onClick={() => void showDetail(trip)} className="text-left font-medium text-primary hover:underline">{trip.tripName}</button><p className="mt-1 text-xs text-muted-foreground">{trip.purpose || "No purpose added"}</p></td><td className="whitespace-nowrap px-4 py-4 text-xs text-muted-foreground">{String(trip.startDate ?? "").slice(0, 10)}{trip.endDate ? ` → ${String(trip.endDate).slice(0, 10)}` : ""}</td><td className="px-4 py-4 text-sm">{trip.location || "—"}</td><td className="px-4 py-4 font-medium">₹{Number(trip.totalExpense ?? 0).toLocaleString("en-IN")}</td><td className="px-4 py-4">{trip.purchaseInvoiceCount ?? 0} · ₹{Number(trip.purchaseInvoiceTotal ?? 0).toLocaleString("en-IN")}</td><td className="px-4 py-4 text-right"><button type="button" onClick={() => setEditing({ _id: trip._id, tripName: String(trip.tripName ?? ""), purpose: String(trip.purpose ?? ""), location: String(trip.location ?? ""), startDate: String(trip.startDate ?? "").slice(0, 10), endDate: String(trip.endDate ?? "").slice(0, 10), notes: String(trip.notes ?? "") })} className="mr-3 text-xs text-primary hover:underline">Edit</button><button type="button" onClick={() => void remove(trip)} className="text-xs text-red-700 hover:underline">Delete</button></td></tr>)}</tbody></table></div></div>;
}
