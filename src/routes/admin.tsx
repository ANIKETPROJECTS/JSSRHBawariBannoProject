...
function PurchaseInvoiceDetail({ invoice, onBack, onEdit, onPaymentUpdated }: { invoice: PurchaseInvoiceRecord; onBack: () => void; onEdit: () => void; onPaymentUpdated: (invoice: PurchaseInvoiceRecord) => void }) {
  return <div>
...
function ProductViewCrudPage() {
  ...
  const visible = products.filter((product) => { const query = search.trim().toLowerCase(); const stock = Number(product.stock ?? 0); const variantLabels = Array.isArray(product.variants) ? (product.variants as RecordItem[]).flatMap((variant) => [variant.color, variant.id]) : []; const selectedCategory = categoryFilter.startsWith("parent:") ? String(product.category ?? "") === categoryFilter.slice(7) : categoryFilter.startsWith("child:") ? String(product.subcategory ?? "") === categoryFilter.slice(6) : true; return (!query || [product.name, product.id, product.fabric, product.category, product.subcategory, categoryName(product.category), categoryName(product.subcategory), ...variantLabels].map((value) => String(value ?? "").toLowerCase()).join(" ").includes(query)) && selectedCategory && (stockFilter === "all" || (stockFilter === "in-stock" && stock > 0) || (stockFilter === "low-stock" && stock > 0 && stock <= 3) || (stockFilter === "out-of-stock" && stock === 0)); });
