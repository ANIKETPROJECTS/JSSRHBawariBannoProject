    ...
    <div className="min-h-screen bg-[#f7f4ef] text-[#2d2520]">
      ...
      <main className={`transition-all ${sidebarOpen ? "lg:ml-64" : "lg:ml-[72px]"}`}>
        ...
        <div className="mx-auto max-w-7xl p-5 md:p-10">
          {tab === "dashboard" ? <Dashboard /> : tab === "inventory" ? <InventoryCrudPage /> : tab === "orders" ? <OrderCrudPage /> : tab === "customers" ? <CustomerCrudPage /> : tab === "settings" ? <SettingsCrudPage /> : tab === "announcements" ? <AnnouncementCrudPage /> : tab === "reviews" ? <ReviewCrudPage /> : <ResourceManager resource={tab} />}
        </div>
      </main>
    </div>
  );
...
function OrdersPage() {
  ...
  function exportPaidOrders() {
    const rows = [["Order ID", "Customer", "Email", "Amount", "Status", "Date"], ...paidOrders.map((order) => [order.orderId ?? "", order.customerName ?? "", order.customerEmail ?? "", String(order.total ?? 0), order.status ?? "", order.createdAt ? new Date(order.createdAt).toLocaleDateString("en-IN") : ""])];
    const csv = rows.map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(",")).join("\n");
    const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" })); link.download = "bawari-banno-paid-orders.csv"; link.click(); URL.revokeObjectURL(link.href);
  }
  ...
  async function changeStatus(order: Order, nextStatus: string) {
    if (!order._id) return;
    try {
      const updated = await api(`/api/admin/orders/${order._id}`, { method: "PATCH", body: JSON.stringify({ status: nextStatus }) });
  ...
  return <div>
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-display text-3xl text-primary">Order Management</h2><p className="mt-1 text-sm text-muted-foreground">Track and manage all customer orders</p></div><button type="button" onClick={exportPaidOrders} className="bg-emerald-600 px-4 py-3 text-xs uppercase tracking-[0.12em] text-white hover:bg-emerald-700">↓ Export paid orders (CSV)</button></div>
    ...
    <div className="mt-6 border border-[#ded5c9] bg-white p-5"><div className="grid gap-3 md:grid-cols-3"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search order #, customer, email…" className="border border-border px-3 py-2.5 text-sm outline-none focus:border-gold" /><select value={sort} onChange={(e) => setSort(e.target.value)} className="border border-border bg-white px-3 py-2.5 text-sm"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="amount">Highest amount</option></select><select value={status} onChange={(e) => setStatus(e.target.value)} className="border border-border bg-white px-3 py-2.5 text-sm"><option value="all">All statuses</option>{statuses.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={payment} onChange={(e) => setPayment(e.target.value)} className="border border-border bg-white px-3 py-2.5 text-sm"><option value="all">All payments</option><option value="paid">Paid</option><option value="pending">Pending</option><option value="failed">Failed</option></select><label className="flex items-center gap-2 text-xs text-muted-foreground">From<input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="min-w-0 border border-border px-2 py-2 text-sm text-primary" /></label><label className="flex items-center gap-2 text-xs text-muted-foreground">To<input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="min-w-0 border border-border px-2 py-2 text-sm text-primary" /></label><button type="button" onClick={clearFilters} className="w-fit border border-border px-3 py-2 text-xs text-muted-foreground hover:text-primary">Clear filters</button></div></div>
    <div className="mt-5 overflow-x-auto border border-[#ded5c9] bg-white"><table className="w-full min-w-[860px] text-left text-sm"><thead className="border-b border-border bg-[#fbf9f6] text-[10px] uppercase tracking-[0.15em] text-muted-foreground"><tr><th className="px-4 py-4">Order #</th><th className="px-4 py-4">Items</th><th className="px-4 py-4">Amount</th><th className="px-4 py-4">Order status</th><th className="px-4 py-4">Payment</th><th className="px-4 py-4">Date</th><th className="px-4 py-4">Action</th></tr></thead><tbody>{loading ? <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">Loading orders…</td></tr> : orders.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">No orders yet. Demo purchases will appear here.</td></tr> : orders.map((order) => <tr key={order._id} className="border-b border-border last:border-0"><td className="px-4 py-4 font-medium text-primary">{order.orderId}</td><td className="px-4 py-4">{order.items?.reduce((sum, item) => sum + item.quantity, 0) ?? 0}</td><td className="px-4 py-4">₹{Number(order.total ?? 0).toLocaleString("en-IN")}</td><td className="px-4 py-4"><select value={order.status ?? "pending"} onChange={(event) => void changeStatus(order, event.target.value)} className="border border-border bg-white px-2 py-1.5 text-xs capitalize outline-none focus:border-gold">{statuses.map((value) => <option key={value} value={value}>{value}</option>)}</select></td><td className="px-4 py-4 text-xs capitalize text-muted-foreground">{order.paymentStatus ?? "pending"}</td><td className="whitespace-nowrap px-4 py-4 text-xs text-muted-foreground">{order.createdAt ? new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}</td><td className="px-4 py-4"><button type="button" onClick={() => setSelected(order)} className="border border-primary px-3 py-1.5 text-xs text-primary hover:bg-primary hover:text-white">View details</button></td></tr>)}</tbody></table></div>
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4" onClick={() => setSelected(null)}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-border bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between border-b border-border pb-5"><div><p className="text-[10px] uppercase tracking-[0.18em] text-gold">Order details</p><h2 className="mt-1 font-display text-2xl text-primary">#{selected.orderId}</h2><p className="mt-1 text-xs text-muted-foreground">{selected.createdAt ? new Date(selected.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}</p></div><button type="button" onClick={() => setSelected(null)} className="text-xl text-muted-foreground" aria-label="Close">×</button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="bg-[#f6f3ef] p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Customer</p><p className="mt-2 font-medium text-primary">{selected.customerName || "Guest checkout"}</p><p className="mt-1 text-sm text-muted-foreground">{selected.customerEmail || "Email not available"}</p><p className="mt-1 text-sm text-muted-foreground">{selected.customerPhone || "Phone not available"}</p></div><div className="bg-[#f6f3ef] p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Payment</p><p className="mt-2 text-sm capitalize">{selected.paymentStatus ?? "pending"} · {selected.paymentMethod ?? "Demo"}</p><p className="mt-2 text-sm text-muted-foreground">Order status: <strong className="capitalize text-primary">{selected.status}</strong></p></div></div><div className="mt-5"><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Update order status</p><div className="mt-2 flex flex-wrap gap-2">{statuses.map((value) => <button key={value} type="button" onClick={() => void changeStatus(selected, value)} className={`border px-3 py-2 text-xs capitalize ${selected.status === value ? "border-primary bg-primary text-white" : "border-border text-primary hover:border-primary"}`}>{value}</button>)}</div></div><div className="mt-6 border-y border-border py-4"><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Order items</p>{selected.items?.map((item, index) => <div key={`${item.productId}-${index}`} className="flex items-center gap-3 border-b border-border py-3 last:border-0"><div className="size-12 overflow-hidden bg-[#f4efe8]">{item.image && <img src={item.image} alt="" className="h-full w-full object-cover" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.name || item.productId}</p><p className="text-xs text-muted-foreground">Qty {item.quantity}{item.price ? ` · ₹${Number(item.price).toLocaleString("en-IN")}` : ""}</p></div><p className="text-sm font-medium">₹{Number((item.price ?? 0) * item.quantity).toLocaleString("en-IN")}</p></div>)}</div><div className="mt-4 space-y-2 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span>₹{Number(selected.subtotal ?? selected.total ?? 0).toLocaleString("en-IN")}</span></div><div className="flex justify-between"><span className="text-muted-foreground">Shipping</span><span>{Number(selected.shipping ?? 0) ? `₹${Number(selected.shipping).toLocaleString("en-IN")}` : "FREE"}</span></div><div className="flex justify-between border-t border-border pt-3 text-base font-medium text-primary"><span>Total</span><span>₹{Number(selected.total ?? 0).toLocaleString("en-IN")}</span></div></div></div></div>}
  </div>;
}
...
function SimpleProductEditor({ initial, categories, onDone }: { initial: RecordItem; categories: RecordItem[]; onDone: () => void }) {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setBusy(true); try { const images = String(form.imageGallery ?? "").split(/\r?\n|,/).map((image) => image.trim()).filter(Boolean); const { _id, imageGallery, subcategory, ...payload } = form; await api(`/api/admin/products${_id ? `/${_id}` : ""}`, { method: _id ? "PUT" : "POST", body: JSON.stringify({ ...payload, images, image: images[0] ?? "" }) }); toast.success("Product saved."); onDone(); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not save product."); } finally { setBusy(false); } }
  return <form onSubmit={submit} className="border border-[#ded5c9] bg-white p-6"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-[0.18em] text-gold">Catalog detail</p><h3 className="mt-1 font-display text-2xl text-primary">{form._id ? "Edit product" : "Add product"}</h3></div><button type="button" onClick={onDone} className="text-xs text-muted-foreground">Cancel</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs text-muted-foreground sm:col-span-2">Product name<input required value={String(form.name ?? "")} onChange={(event) => set("name", event.target.value)} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Product ID<input required value={String(form.id ?? "")} onChange={(event) => set("id", event.target.value)} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Category<select required value={String(form.category ?? "")} onChange={(event) => set("category", event.target.value)} className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm"><option value="">Choose a category</option>{categories.map((category) => <option key={category._id} value={String(category.slug)}>{String(category.label ?? category.slug)}</option>)}</select></label><label className="text-xs text-muted-foreground">Fabric<input value={String(form.fabric ?? "")} onChange={(event) => set("fabric", event.target.value)} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Price (₹)<input required type="number" min="0" value={Number(form.price ?? 0)} onChange={(event) => set("price", Number(event.target.value))} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Stock quantity<input required type="number" min="0" value={Number(form.stock ?? 0)} onChange={(event) => set("stock", Number(event.target.value))} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground sm:col-span-2">Image URLs<textarea required value={String(form.imageGallery ?? "")} onChange={(event) => set("imageGallery", event.target.value)} rows={4} placeholder="One URL per line" className="mt-1 w-full resize-y border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground sm:col-span-2">Description<textarea required value={String(form.description ?? "")} onChange={(event) => set("description", event.target.value)} rows={5} className="mt-1 w-full resize-y border border-border px-3 py-2.5 text-sm" /></label></div><div className="mt-6 flex gap-5 border-t border-border pt-5 text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={form.published !== false} onChange={(event) => set("published", event.target.checked)} /> Published on storefront</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.featured === true} onChange={(event) => set("featured", event.target.checked)} /> Featured product</label></div><button disabled={busy} className="mt-6 w-full bg-primary px-4 py-3 text-xs uppercase tracking-[0.14em] text-white disabled:opacity-50">{busy ? "Saving product…" : "Save product"}</button></form>;
}
...
function ProductViewCrudPage() {
  ...
  const categoryOptions = useMemo(() => {
    const options = new Map<string, string>();
    categories.filter((category) => !category.parentSlug).forEach((category) => { const slug = String(category.slug ?? "").trim(); if (slug) options.set(slug, String(category.label ?? category.name ?? slug)); });
    products.forEach((product) => { const slug = String(product.category ?? "").trim(); if (slug && !options.has(slug)) options.set(slug, slug); });
    return [...options.entries()].map(([value, label]) => ({ value, label })).sort((a, b) => a.label.localeCompare(b.label));
  }, [categories, products]);
  const categoryName = (value: unknown) => categoryOptions.find((item) => item.value === String(value))?.label ?? String(value ?? "Unassigned");
  const visible = products.filter((product) => { const query = search.trim().toLowerCase(); const stock = Number(product.stock ?? 0); return (!query || [product.name, product.id, product.fabric, product.category, categoryName(product.category)].map((value) => String(value ?? "").toLowerCase()).join(" ").includes(query)) && (categoryFilter === "all" || String(product.category ?? "") === categoryFilter) && (stockFilter === "all" || (stockFilter === "in-stock" && stock > 0) || (stockFilter === "low-stock" && stock > 0 && stock <= 3) || (stockFilter === "out-of-stock" && stock === 0)); });
  ...
  if (editing) return <div><button type="button" onClick={() => setEditing(null)} className="mb-5 text-sm text-primary hover:underline">← Back to products</button><div className="max-w-4xl"><SimpleProductEditor initial={editing} categories={categories.filter((category) => !category.parentSlug)} onDone={() => { setEditing(null); void load(); }} /></div></div>;
