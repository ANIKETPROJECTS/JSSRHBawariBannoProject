...
function ProductViewPanel({ product, categoryName, onClose, onReorder, onProductUpdated }: { product: RecordItem; categoryName: (value: unknown) => string; onClose: () => void; onReorder?: (draft: PurchaseInvoiceRecord) => void; onProductUpdated?: (product: RecordItem) => void }) {
  ...
  return <div className="border border-[#ded5c9] bg-white p-6">
     <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border pb-5"><div><p className="text-[10px] uppercase tracking-[0.18em] text-gold">Product details</p><h2 className="mt-1 font-display text-3xl text-primary">{String(product.name ?? product.id)}</h2><p className="mt-1 text-xs text-muted-foreground">{String(product.id ?? "No product ID")}</p></div><div className="flex items-center gap-3">{reorderBatch && <button type="button" onClick={reorderFromProduct} className="border border-primary px-3 py-2 text-xs text-primary hover:bg-primary hover:text-white">Reorder</button>}<button type="button" onClick={onClose} className="text-sm text-primary hover:underline">← Back</button></div></div>
