...
type ProductFinancialBatch = { id: string; variantId: string; variantColor: string; sourceType: string; sourceLabel: string; invoiceNumber: string; vendorId?: string; vendorProductCode?: string; vendorName: string; receivedDate?: string | null; quantityReceived: number; quantityRemaining: number; quantitySold: number; costPricePerUnit: number | null; costKnown: boolean; remainingCostValue: number | null; sellingValue: number; margin: number | null; marginPercent: number | null };
...
async function uploadExpenseReceipt(expenseId: string, file: File) {
  const form = new FormData();
  form.append("receipt", file);
  const response = await fetch(`/api/admin/expenses/${expenseId}/receipt`, { method: "POST", body: form });
  const result = await response.json().catch(() => ({}));
...
function ProductImagePicker({ value, onChange, label = "Cover image URL", required = false }: { value: string; onChange: (value: string) => void; label?: string; required?: boolean }) {
  ...
  async function choose(file: File) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Could not upload the product image.");
    } finally {
      setBusy(false);
    }
  }
  return <div className="text-xs text-muted-foreground"><label>{label}<input required={required} value={value} onChange={(event) => onChange(event.target.value)} placeholder="https://… or upload below" className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="mt-2 block">Or upload image<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void choose(file); event.currentTarget.value = ""; }} className="mt-1 block w-full text-xs file:mr-2 file:border-0 file:bg-primary file:px-3 file:py-2 file:text-xs file:text-white disabled:opacity-50" /><span className="mt-1 block text-[11px]">{busy ? "Uploading…" : "JPG, PNG, WEBP, or GIF · max 8 MB"}</span></label></div>;
}

function adminSlug(value: unknown) {
  return String(value ?? "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
type ExpenseRecord = RecordItem & {
  ...
  receiptFile?: { id?: string; filename?: string; contentType?: string; size?: number; uploadedAt?: string };
};
...
type ExpenseForm = {
  ...
  vendorId: string;
  tripId: string;
  receiptFile?: ExpenseRecord["receiptFile"];
};
...
function ExpensesPage() {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      const saved = await api(editing._id ? `/api/admin/expenses/${editing._id}` : "/api/admin/expenses", { method: editing._id ? "PUT" : "POST", body: JSON.stringify(payload) });
      if (saved._id && removeReceipt && editing._id) await api(`/api/admin/expenses/${saved._id}/receipt`, { method: "DELETE" });
      if (saved._id && receiptUpload) await uploadExpenseReceipt(saved._id, receiptUpload);
...
function PurchaseInvoiceEditor({ initial, onDone }: { initial: PurchaseInvoiceRecord; onDone: (saved?: PurchaseInvoiceRecord) => void | Promise<void> }) {
  ...
  const [newProduct, setNewProduct] = useState({ code: "", name: "", price: 0, category: "", newCategoryName: "", newCategorySlug: "", image: "", color: "" });
  ...
  function startCreatingLine(index: number) {
    const line = lines[index];
    const defaultName = String(line?.itemName ?? "").trim();
    setCreatingLine(index);
    setNewProduct({
      code: adminSlug(defaultName),
      name: defaultName,
      price: 0,
      category: String(categories.find((category) => !category.parentSlug)?.slug ?? ""),
      newCategoryName: "",
      newCategorySlug: "",
      image: "",
      color: "",
  ...
  async function createProductFromLine(index: number) {
    ...
    const code = adminSlug(newProduct.code);
    if (!name || !code || !newProduct.image.trim()) {
      toast.error("Add a product name, unique product code, and cover image before creating the match.");
      return;
    }
    try {
      let category = String(newProduct.category ?? "").trim();
      if (category === "__new__") {
        const label = String(newProduct.newCategoryName ?? "").trim();
        const slug = adminSlug(newProduct.newCategorySlug || label);
        if (!label || !slug) throw new Error("Enter a name for the new category.");
        if (categories.some((item) => String(item.slug ?? "").toLowerCase() === slug.toLowerCase())) {
          throw new Error("A category with this code already exists. Choose it from the category list.");
        }
        const createdCategory = await api("/api/admin/categories", {
          method: "POST",
          body: JSON.stringify({
            label,
            slug,
            description: `Explore our ${label.toLowerCase()} collection.`,
            order: categories.filter((item) => !item.parentSlug).length,
            published: true,
          }),
        });
        category = String(createdCategory.slug ?? slug);
        setCategories((current) => [...current, createdCategory]);
      }
      if (!category) throw new Error("Choose an existing category or create a new one.");
      ...
      const payload = {
        ...
        variants: color ? [{ color, stock: 0, reorderLevel: 3, image: newProduct.image.trim(), images: [] }] : [],
        published: true,
      };
  ...
  return <form noValidate onSubmit={(event) => { event.preventDefault(); void submit(); }} className="max-w-6xl border border-[#ded5c9] bg-white p-6">
        ...
        <div className="min-w-[920px]">
          ...
          {lines.map((line, index) => {
            ...
            return <div key={line._id ?? index} className="grid grid-cols-[1.6fr_1fr_1.1fr_0.8fr_1fr_36px] gap-3 border-b border-border px-3 py-3 last:border-0">
               <div><div className="flex flex-col gap-2"><select required value={line.productId} onChange={(event) => { const next = products.find((entry) => String(entry.id ?? entry._id) === event.target.value); updateLine(index, { productId: event.target.value, variantId: "", itemName: String(next?.name ?? line.itemName ?? "") }); }} onBlur={() => matchLine(index)} className="w-full border border-border bg-white px-2 py-2 text-sm"><option value="">Choose existing product</option>{products.map((entry) => <option key={String(entry.id ?? entry._id)} value={String(entry.id ?? entry._id)}>{String(entry.name ?? entry.id)} · {String(entry.id ?? "no-code")}</option>)}</select><button type="button" onClick={() => startCreatingLine(index)} className="w-full border border-primary px-2 py-2 text-[10px] uppercase tracking-[0.08em] text-primary hover:bg-primary hover:text-white">+ New product</button></div><input value={line.itemName} onChange={(event) => updateLine(index, { itemName: event.target.value, productId: line.productId && String(products.find((entry) => String(entry.id ?? entry._id) === line.productId)?.name ?? "").toLowerCase() === event.target.value.trim().toLowerCase() ? line.productId : "" })} onBlur={() => matchLine(index)} placeholder="Optional: supplier item / paste name to match" className="mt-2 w-full border border-border px-2 py-2 text-xs" />{!line.productId && line.itemName.trim() && <div className="mt-2 space-y-1">{products.filter((entry) => String(entry.name ?? "").toLowerCase().includes(line.itemName.trim().toLowerCase()) || String(entry.id ?? "").toLowerCase().includes(line.itemName.trim().toLowerCase())).slice(0, 3).map((entry) => <button key={String(entry.id ?? entry._id)} type="button" onClick={() => updateLine(index, { productId: String(entry.id ?? entry._id), variantId: "", itemName: String(entry.name ?? "") })} className="block w-full truncate border border-border bg-[#fbf9f6] px-2 py-1 text-left text-[11px] text-primary hover:border-primary">Match: {String(entry.name ?? entry.id)} · {String(entry.id ?? "")}</button>)}<button type="button" onClick={() => startCreatingLine(index)} className="block w-full border border-dashed border-gold px-2 py-1 text-left text-[11px] text-gold hover:bg-gold/5">Create new product from this item</button></div>}</div>
    ...
    {locked && <div className="mt-7 border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">This invoice is posted and locked. Its purchase stock batch has been created and future corrections must use an explicit correction flow.</div>}
      {creatingLine !== null && <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/40 p-5" onClick={() => setCreatingLine(null)}><div className="my-8 w-full max-w-xl border border-border bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.18em] text-gold">New product</p><h3 className="mt-1 font-display text-2xl text-primary">Add product to this invoice</h3><p className="mt-2 text-sm text-muted-foreground">Create the product once, then this invoice line will use it automatically. The invoice quantity becomes the first FIFO stock batch when you post it.</p></div><button type="button" onClick={() => setCreatingLine(null)} className="text-sm text-primary">Cancel</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs text-muted-foreground sm:col-span-2">Product name<input required value={newProduct.name} onChange={(event) => { const name = event.target.value; setNewProduct({ ...newProduct, name, code: newProduct.code || adminSlug(name) }); }} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Product code<input required maxLength={72} value={newProduct.code} onChange={(event) => setNewProduct({ ...newProduct, code: adminSlug(event.target.value) })} placeholder="Generated from product name" className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /><span className="mt-1 block text-[11px] text-muted-foreground">This code becomes the permanent product ID used in stock, invoices, and inventory logs.</span></label><label className="text-xs text-muted-foreground">Selling price (₹)<input required min="0" type="number" value={newProduct.price} onChange={(event) => setNewProduct({ ...newProduct, price: Number(event.target.value) })} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Category<select required value={newProduct.category} onChange={(event) => setNewProduct({ ...newProduct, category: event.target.value })} className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm"><option value="">Choose a category</option>{categories.filter((category) => !category.parentSlug).map((category) => <option key={String(category.slug)} value={String(category.slug)}>{String(category.label ?? category.slug)}</option>)}<option value="__new__">＋ Create new category</option></select></label>{newProduct.category === "__new__" && <div className="sm:col-span-2 grid gap-3 border border-gold/40 bg-[#fbf9f6] p-4 sm:grid-cols-2"><label className="text-xs text-muted-foreground">New category name<input required value={newProduct.newCategoryName} onChange={(event) => setNewProduct({ ...newProduct, newCategoryName: event.target.value, newCategorySlug: newProduct.newCategorySlug || adminSlug(event.target.value) })} placeholder="e.g. Chanderi" className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm" /></label><label className="text-xs text-muted-foreground">Category code<input required value={newProduct.newCategorySlug} onChange={(event) => setNewProduct({ ...newProduct, newCategorySlug: adminSlug(event.target.value) })} placeholder="chanderi" className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm" /></label><p className="text-[11px] text-muted-foreground sm:col-span-2">This category will be available immediately for this product and future purchases.</p></div>}<div className="text-xs text-muted-foreground sm:col-span-2"><label>Cover image URL<input required value={newProduct.image} onChange={(event) => setNewProduct({ ...newProduct, image: event.target.value })} placeholder="https://… or upload below" className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><label className="mt-2 block">Or upload image<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => { const file = event.target.files?.[0]; if (file) void chooseNewProductImage(file); event.currentTarget.value = ""; }} className="mt-1 block w-full text-xs file:mr-2 file:border-0 file:bg-primary file:px-3 file:py-2 file:text-xs file:text-white" /><span className="mt-1 block text-[11px]">JPG, PNG, WEBP, or GIF · max 8 MB</span></label></div><label className="text-xs text-muted-foreground">Colour variant (optional)<input value={newProduct.color} onChange={(event) => setNewProduct({ ...newProduct, color: event.target.value })} placeholder="e.g. ivory" className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label></div><button type="button" onClick={() => void createProductFromLine(creatingLine)} className="mt-6 bg-primary px-5 py-3 text-xs uppercase tracking-[0.14em] text-white">Create and match line</button></div></div>}
  </form>;
}
...
function PurchaseInvoiceDocumentUploader({ invoice, onUpdated }: { invoice: PurchaseInvoiceRecord; onUpdated: (invoice: PurchaseInvoiceRecord) => void }) {
  const [file, setFile] = useState<File | null>(null);
  const [busy, setBusy] = useState(false);
  async function upload() {
    if (!invoice._id || !file) return;
    setBusy(true);
    try {
      const form = new FormData();
      form.append("document", file);
      const updated = await api(`/api/admin/purchase-invoices/${invoice._id}/document`, { method: "POST", body: form });
      onUpdated(updated);
      setFile(null);
      toast.success("Original invoice document attached.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not upload the invoice document.");
    } finally {
      setBusy(false);
    }
  }
  return <section className="mt-6 border border-amber-200 bg-amber-50 p-5"><p className="text-[10px] uppercase tracking-[0.16em] text-amber-800">Missing original document</p><h3 className="mt-1 font-display text-2xl text-amber-950">Attach invoice file</h3><p className="mt-2 text-sm text-amber-900">This invoice is already posted, so its stock and financial lines remain locked. You can still attach the missing PDF or image for audit reference.</p><div className="mt-4 flex flex-wrap items-end gap-3"><label className="min-w-64 flex-1 text-xs text-amber-900">PDF or image<input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,.gif,application/pdf,image/jpeg,image/png,image/webp,image/gif" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="mt-1 block w-full border border-amber-300 bg-white px-3 py-2.5 text-sm file:mr-3 file:border-0 file:bg-[#f4efe8] file:px-3 file:py-1.5 file:text-xs" /></label><button type="button" disabled={!file || busy} onClick={() => void upload()} className="bg-primary px-4 py-2.5 text-xs uppercase tracking-[0.12em] text-white disabled:opacity-40">{busy ? "Uploading…" : "Attach document"}</button></div>{file && <p className="mt-2 text-xs text-amber-900">Selected: {file.name}</p>}</section>;
}
...
function AuditLogsPage() {
  ...
  useEffect(() => {
    const params = new URLSearchParams({ search, entityType, action, from, to });
    setLoading(true);
...
function ResourceManager({ resource, onReorder }: { resource: Exclude<Tab, "dashboard" | "inventory" | "settings" | "customers" | "reviews" | "coupons">; onReorder?: (draft: PurchaseInvoiceRecord) => void }) {
  ...
  if (resource === "products") return <ProductViewCrudPage onReorder={onReorder} />;
...
function SimpleProductEditor({ initial, categories, onDone }: { initial: RecordItem; categories: RecordItem[]; onDone: () => void }) {
  ...
  const [form, setForm] = useState<RecordItem>(() => ({
    ...
    hsnCode: String(initial.hsnCode ?? ""),
    gstRate: initial.gstRate == null ? "" : Number(initial.gstRate),
      ...
      await api(`/api/admin/products${_id ? `/${_id}` : ""}`, {
        ...
        body: JSON.stringify({
          ...
          countryOfOrigin: String(form.countryOfOrigin ?? "").trim() || "India",
           ...
           gstRate: form.gstRate === "" ? 0 : Number(form.gstRate ?? 0),
        }),
      });
      toast.success("Product saved."); onDone();
    } catch (error) { toast.error(error instanceof Error ? error.message : "Could not save product."); }
      ...
      <div className="mt-6 space-y-6">
        <section className="grid gap-4 sm:grid-cols-2">
          <label className="text-xs text-muted-foreground sm:col-span-2">Product name<input required value={String(form.name ?? "")} onChange={(event) => set("name", event.target.value)} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label>
           ...
           <label className="text-xs text-muted-foreground">HSN code<input value={String(form.hsnCode ?? "")} onChange={(event) => set("hsnCode", event.target.value)} placeholder="5208" className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label>
           <label className="text-xs text-muted-foreground">GST on sale (%)<input type="number" min="0" max="100" step="0.01" value={form.gstRate === "" ? "" : Number(form.gstRate ?? 0)} onChange={(event) => set("gstRate", event.target.value === "" ? "" : Number(event.target.value))} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label>
...
function ProductViewPanel({ product, categoryName, onClose, onEdit, onReorder, onProductUpdated }: { product: RecordItem; categoryName: (value: unknown) => string; onClose: () => void; onEdit?: () => void; onReorder?: (draft: PurchaseInvoiceRecord) => void; onProductUpdated?: (product: RecordItem) => void }) {
  const images = Array.isArray(product.images) && product.images.length ? product.images : [product.image];
  const variants = Array.isArray(product.variants) ? product.variants as RecordItem[] : [];
  const [activity, setActivity] = useState<RecordItem[]>([]);
  const [activityLoading, setActivityLoading] = useState(true);
  const [financials, setFinancials] = useState<ProductFinancials | null>(null);
  const [financialsLoading, setFinancialsLoading] = useState(true);
  const [sellingPrice, setSellingPrice] = useState(Number(product.price ?? 0));
  const [priceBusy, setPriceBusy] = useState(false);
  const productId = String(product.id ?? "");
  useEffect(() => { setSellingPrice(Number(product.price ?? 0)); }, [product.price]);
  useEffect(() => {
    ...
    Promise.all([
      ...
      api(`/api/admin/product-financials?productId=${encodeURIComponent(productId)}`),
    ])
      .then(([events, details]) => {
        setActivity(Array.isArray(events) ? events : []);
        setFinancials(details as ProductFinancials);
      })
      .catch((error) => toast.error(error instanceof Error ? error.message : "Could not load product financials."))
      .finally(() => { setActivityLoading(false); setFinancialsLoading(false); });
  }, [productId]);
  const formatMoney = (value: number | null | undefined) => value == null ? "Unknown" : `₹${Number(value).toLocaleString("en-IN", { maximumFractionDigits: 2 })}`;
  const reorderBatch = financials?.batches
    .filter((batch) => batch.sourceType === "purchase" && batch.vendorId && batch.costPricePerUnit != null)
    .sort((a, b) => String(b.receivedDate ?? "").localeCompare(String(a.receivedDate ?? "")))[0];
  async function saveSellingPrice() {
    if (!product._id || !Number.isFinite(sellingPrice) || sellingPrice < 0) {
      toast.error("Enter a valid selling price.");
      return;
    }
    setPriceBusy(true);
    try {
      const updated = await api(`/api/admin/products/${product._id}`, {
        method: "PUT",
        body: JSON.stringify({ ...product, price: sellingPrice, originalPrice: sellingPrice, discountType: "percentage", discountValue: 0 }),
      });
      onProductUpdated?.(updated);
      toast.success("Selling price updated.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not update selling price.");
    } finally {
      setPriceBusy(false);
    }
  }
  function reorderFromProduct() {
    if (!onReorder || !reorderBatch) return;
    onReorder({
      ...emptyInvoice,
      vendorId: reorderBatch.vendorId,
      lines: [{
        productId,
        variantId: reorderBatch.variantId,
        vendorProductCode: reorderBatch.vendorProductCode ?? "",
        itemName: reorderBatch.variantColor || String(product.name ?? productId),
        quantityPurchased: Math.max(Number(product.reorderLevel ?? 1) - Number(product.stock ?? 0), 1),
        costPricePerUnit: Number(reorderBatch.costPricePerUnit ?? 0),
      }],
    });
  }
  return <div className="border border-[#ded5c9] bg-white p-6">
    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border pb-5"><div><p className="text-[10px] uppercase tracking-[0.18em] text-gold">Product details</p><h2 className="mt-1 font-display text-3xl text-primary">{String(product.name ?? product.id)}</h2><p className="mt-1 text-xs text-muted-foreground">Product code: {String(product.id ?? "Not assigned")}</p></div><div className="flex items-center gap-3">{reorderBatch && <button type="button" onClick={reorderFromProduct} className="border border-primary px-3 py-2 text-xs text-primary hover:bg-primary hover:text-white">Reorder</button>}{onEdit && <button type="button" onClick={onEdit} className="border border-border px-3 py-2 text-xs text-primary hover:border-primary">Edit product</button>}<button type="button" onClick={onClose} className="text-sm text-primary hover:underline">← Back</button></div></div>
     ...
     <section className="mt-6 border border-border bg-[#fbf9f6] p-4"><div className="flex flex-wrap items-end gap-3"><label className="min-w-56 flex-1 text-xs text-muted-foreground">Adjust selling price (₹)<input type="number" min="0" step="1" value={sellingPrice} onChange={(event) => setSellingPrice(Number(event.target.value))} className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm" /></label><button type="button" disabled={priceBusy} onClick={() => void saveSellingPrice()} className="bg-primary px-4 py-2.5 text-xs uppercase tracking-[0.12em] text-white disabled:opacity-50">{priceBusy ? "Saving…" : "Save selling price"}</button></div><p className="mt-2 text-[11px] text-muted-foreground">This updates the customer-facing selling price and resets any product discount.</p></section>
     {variants.length > 0 && <section className="mt-7 border-t border-border pt-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.16em] text-gold">Colour stock breakdown</p><h3 className="mt-1 font-display text-2xl text-primary">Variant stock</h3></div><p className="text-xs text-muted-foreground">Variant total should equal the product total.</p></div><div className="mt-4 overflow-x-auto border border-border"><table className="w-full min-w-[520px] text-left text-sm"><thead className="border-b border-border bg-[#fbf9f6] text-[10px] uppercase tracking-[0.12em] text-muted-foreground"><tr><th className="px-3 py-3">Colour</th><th className="px-3 py-3">Variant ID</th><th className="px-3 py-3 text-right">Stock</th></tr></thead><tbody>{variants.map((variant) => <tr key={String(variant.id ?? variant.color)} className="border-b border-border last:border-0"><td className="px-3 py-3 font-medium text-primary">{String(variant.color ?? "Unnamed colour")}</td><td className="px-3 py-3 text-xs text-muted-foreground">{String(variant.id ?? "—")}</td><td className="px-3 py-3 text-right font-medium text-primary">{Number(variant.stock ?? 0)}</td></tr>)}<tr className="bg-[#fbf9f6] font-medium"><td className="px-3 py-3" colSpan={2}>Total product stock</td><td className="px-3 py-3 text-right">{Number(product.stock ?? 0)}</td></tr></tbody></table></div></section>}
      <section className="mt-7 border-t border-border pt-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.16em] text-gold">Inventory valuation</p><h3 className="mt-1 font-display text-2xl text-primary">FIFO financial detail</h3></div><p className="max-w-md text-right text-xs text-muted-foreground">Purchase-backed stock is valued at recorded cost. Opening-balance stock remains explicitly uncosted.</p></div>{financialsLoading ? <p className="mt-4 text-sm text-muted-foreground">Loading financial detail…</p> : !financials ? <p className="mt-4 border border-dashed border-[#cfc3b5] bg-[#fbf9f6] p-4 text-sm text-muted-foreground">Financial detail is unavailable for this product.</p> : <><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="bg-[#fbf9f6] p-4"><p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Remaining units</p><p className="mt-2 text-xl font-medium text-primary">{financials.remainingQuantity}</p><p className="mt-1 text-xs text-muted-foreground">{financials.trackedStock} tracked · {financials.untrackedStock} untracked</p></div><div className="bg-[#fbf9f6] p-4"><p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Purchase cost value</p><p className="mt-2 text-xl font-medium text-primary">{formatMoney(financials.remainingInventoryAtPurchaseCost)}</p><p className="mt-1 text-xs text-muted-foreground">{financials.purchaseBackedQuantity} purchase-backed units</p></div><div className="bg-[#fbf9f6] p-4"><p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Selling-price value</p><p className="mt-2 text-xl font-medium text-primary">{formatMoney(financials.inventoryAtSellingPrice)}</p><p className="mt-1 text-xs text-muted-foreground">At ₹{financials.sellingPricePerUnit.toLocaleString("en-IN")} per unit</p></div><div className="bg-[#fbf9f6] p-4"><p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Costed batch margin</p><p className="mt-2 text-xl font-medium text-primary">{formatMoney(financials.costedBatchMargin)}</p><p className="mt-1 text-xs text-muted-foreground">{financials.costedBatchMarginPercent == null ? "No costed batches" : `${financials.costedBatchMarginPercent.toLocaleString("en-IN", { maximumFractionDigits: 2 })}% of selling value`}</p></div></div>{financials.unknownCostQuantity > 0 && <div className="mt-4 border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><strong>{financials.unknownCostQuantity} units have unknown cost.</strong> Their selling-price value is {formatMoney(financials.unknownCostSellingValue)}, but they are excluded from purchase-cost and margin calculations.</div>}<div className="mt-5 overflow-x-auto border border-border"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b border-border bg-[#fbf9f6] text-[10px] uppercase tracking-[0.12em] text-muted-foreground"><tr><th className="px-3 py-3">FIFO batch</th><th className="px-3 py-3">Received</th><th className="px-3 py-3 text-right">Remaining</th><th className="px-3 py-3 text-right">Cost / unit</th><th className="px-3 py-3 text-right">Cost value</th><th className="px-3 py-3 text-right">Selling value</th><th className="px-3 py-3 text-right">Margin</th></tr></thead><tbody>{financials.batches.length === 0 ? <tr><td colSpan={7} className="px-3 py-6 text-center text-sm text-muted-foreground">No remaining FIFO batches are recorded for this product.</td></tr> : financials.batches.map((batch) => <tr key={batch.id} className="border-b border-border last:border-0"><td className="px-3 py-3"><p className="font-medium text-primary">{batch.variantColor ? `${batch.variantColor} · ` : ""}{batch.sourceLabel}</p><p className="mt-1 text-xs text-muted-foreground">{batch.invoiceNumber || batch.vendorName || (batch.sourceType === "opening_balance" ? "Legacy opening stock" : "Source details unavailable")}</p></td><td className="px-3 py-3 text-xs text-muted-foreground">{batch.receivedDate ? new Date(batch.receivedDate).toLocaleDateString("en-IN") : "—"}</td><td className="px-3 py-3 text-right font-medium text-primary">{batch.quantityRemaining}</td><td className="px-3 py-3 text-right">{batch.costKnown ? formatMoney(batch.costPricePerUnit) : <span className="text-amber-700">Unknown</span>}</td><td className="px-3 py-3 text-right">{formatMoney(batch.remainingCostValue)}</td><td className="px-3 py-3 text-right">{formatMoney(batch.sellingValue)}</td><td className="px-3 py-3 text-right">{formatMoney(batch.margin)}</td></tr>)}</tbody></table></div></>}</section>
...
function ProductViewCrudPage({ onReorder }: { onReorder?: (draft: PurchaseInvoiceRecord) => void }) {
  ...
  if (viewing) return <ProductViewPanel product={viewing} categoryName={categoryName} onClose={() => setViewing(null)} onEdit={() => { setEditing(viewing); setViewing(null); }} onReorder={onReorder} onProductUpdated={(updated) => { setViewing(updated); setProducts((current) => current.map((item) => item._id === updated._id ? updated : item)); }} />;
...
function InventoryEditor({ initial, products, onDone }: { initial: InventoryEvent; products: RecordItem[]; onDone: () => void }) {
  const [form, setForm] = useState({ productId: String(initial.productId ?? ""), variantId: String(initial.variantId ?? ""), eventType: String(initial.eventType ?? "manual"), quantity: Number(initial.quantity ?? 0), reason: String((initial as InventoryEvent & { reason?: string }).reason ?? "") });
  const [busy, setBusy] = useState(false);
  const selectedProduct = products.find((product) => String(product.id ?? "") === form.productId);
  const productVariants = Array.isArray(selectedProduct?.variants) ? selectedProduct.variants as RecordItem[] : [];
  async function submit(event: React.FormEvent) {
    event.preventDefault(); setBusy(true);
    try {
      if (!form.productId) throw new Error("Choose a product.");
      if (productVariants.length && !form.variantId) throw new Error("Choose a colour variant.");
  ...
  return <form onSubmit={submit} className="max-w-xl border border-[#ded5c9] bg-white p-6"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-[0.18em] text-gold">Inventory record</p><h2 className="mt-1 font-display text-3xl text-primary">{initial._id ? "Edit movement" : "Add movement"}</h2><p className="mt-2 text-sm text-muted-foreground">Manual movements update the product or selected colour stock and appear in the inventory log.</p></div><button type="button" onClick={onDone} className="text-xs text-muted-foreground">Cancel</button></div><label className="mt-6 block text-xs text-muted-foreground">Product<select required disabled={Boolean(initial._id)} value={form.productId} onChange={(event) => setForm({ ...form, productId: event.target.value, variantId: "" })} className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm disabled:bg-[#f4efe8]"><option value="">Choose a product</option>{products.map((product) => <option key={product._id} value={String(product.id)}>{String(product.name ?? product.id)} · {String(product.id ?? "")}</option>)}</select></label>{productVariants.length > 0 && <label className="mt-4 block text-xs text-muted-foreground">Colour variant<select required disabled={Boolean(initial._id)} value={form.variantId} onChange={(event) => setForm({ ...form, variantId: event.target.value })} className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm disabled:bg-[#f4efe8]"><option value="">Choose a colour</option>{productVariants.map((variant) => <option key={String(variant.id)} value={String(variant.id)}>{String(variant.color ?? variant.id)} · Stock {Number(variant.stock ?? 0)}</option>)}</select></label>}{productVariants.length === 0 && form.productId && <p className="mt-4 text-xs text-muted-foreground">This product uses one product-level stock pool.</p>}<label className="mt-4 block text-xs text-muted-foreground">Event type<select value={form.eventType} onChange={(event) => setForm({ ...form, eventType: event.target.value })} className="mt-1 w-full border border-border bg-white px-3 py-2.5 text-sm"><option value="manual">Manual adjustment</option><option value="purchase">Purchase</option></select></label><label className="mt-4 block text-xs text-muted-foreground">Quantity change<input required type="number" step="1" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: Number(event.target.value) })} className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /><span className="mt-1 block text-[11px]">Use a positive number to add stock or a negative number to remove it.</span></label><label className="mt-4 block text-xs text-muted-foreground">Reason<input maxLength={200} value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} placeholder="Restock, correction, damaged item…" className="mt-1 w-full border border-border px-3 py-2.5 text-sm" /></label><button disabled={busy} className="mt-6 w-full bg-primary px-4 py-3 text-xs uppercase tracking-[0.14em] text-white disabled:opacity-50">{busy ? "Saving…" : "Save movement"}</button></form>;
}
